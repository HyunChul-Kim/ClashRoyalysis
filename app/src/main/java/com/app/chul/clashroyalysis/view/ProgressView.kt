...
class ProgressView @JvmOverloads constructor(
    ...
    init {
        ...
        attrs?.let {
            ...
            mPoint = typedArray.getInteger(R.styleable.ProgressView_points, mPoint)
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        var textPoint = mPoint.toString()
        mTextPaint.getTextBounds(textPoint, 0, textPoint.length, mTextRect)

        var x = canvas?.width!! / 2f - mTextRect.width() / 2f
        var y = mArcRect.centerY() as Int - ((mTextPaint.descent() + mTextPaint.ascent()) / 2)
        canvas?.drawText(mPoint.toString(), x, y, mTextPaint)

        canvas?.drawArc(mArcRect, 0f, 360f, false, mArcPaint)
        canvas?.drawArc(mArcRect, 0f, mProgressSweep, false, mProgressPaint)
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        var arcWidth = min - paddingLeft
        mArcRadius = arcWidth / 2
    ...
    fun setProgress(value: Int) {
        mPoint = value
        mProgressSweep = (value as Float / mMax) * 360f
    }
}